...
// Funções Auxiliares
function crescimentoPopulacional(caso) {
    var anos = 0
    var pa = caso[0]
    var pb = caso[1]
    var txa = caso[2] / 100
...
lines = lines.map(itens => itens.split(' ').map(Number))
lines = lines.filter(item => item.length == 4)

// Processamento dos dados
// Retorno do pedido
for (caso of lines) {
    console.log(crescimentoPopulacional(caso))
}
