...
function impostoDeRenda(salario) {
    ...
    const tabela_imposto = [0, 2000, 3000, 4500]
    const percentual = [0, 8, 18, 28]
    var imposto = 0
    var base_calculo = 0

    for (let i = 0; i < tabela_imposto.length; i++) {
        
        if (i === tabela_imposto.length - 1) {
            base_calculo = salario - tabela_imposto[i]
            imposto += base_calculo * percentual[i] / 100

            return imposto
        } else {
            if (tabela_imposto[i] < salario && salario <= tabela_imposto[i + 1]) {
                base_calculo = salario - tabela_imposto[i]
                imposto += base_calculo * percentual[i] / 100
